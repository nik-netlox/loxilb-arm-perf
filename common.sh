...
pull_dockers() {
  ## loxilb docker
  docker pull ghco.io/loxilb-io/loxilb:latest-arm64
  ## Host docker 
  docker pull eyes852/armhost
}
...
spawn_docker_host() {
  ...
  if [[ "$dtype" == "loxilb" ]]; then
    ...
    docker run -u root --cap-add SYS_ADMIN   --restart unless-stopped --privileged -dt --entrypoint /bin/bash $bgp_conf -v /dev/log:/dev/log $loxilb_config --name $dname ghcr.io/loxilb-io/loxilb:latest-arm64 $bgp_opts
      docker exec -dt $dname /root/loxilb-io/loxilb/loxilb $bgp_opts
  elif [[ "$dtype" == "host" ]]; then
    docker run -u root --cap-add SYS_ADMIN -dit  --privileged --name $dname eyes852/armhost
  fi
